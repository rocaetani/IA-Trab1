...
public class Board
{
    ...
    {
        _numPlays = 9;
        for(int position = 0; position < _board.Count; position++)
        {
            Remove(position);
        }
    }
    ...
    public int Score(Symbol player) {
        //return the score acording to a player win or not
        // X - Win: +10 / O - Win: -10 / Tie: -1 / StillPlaing = 0

        if (VerifyWinPossibilities(player)) {
            return 10;
        }
        if (VerifyWinPossibilities(player.GetOther())) {
            return -10;
        }
